# a = [3, 4, 5, 2, 7, 8]
# b = [7, 9, 2, 4, 5, 1]
# c = [5, 7, 3, 4, 5, 9]
#
# print(*map(lambda x, y, z: x + y + z, a, b, c))


# a = [3, 4, 5, 2, 7, 8]
# b = [7, 9, 2, 4, 5, 1]
# c = [5, 7, 3, 4, 5, 9]
#
# print(list(map(lambda x, y, z: x + y + z, a, b, c)))

# dict_a = [{'name': 'python', 'points':10}, {'name': 'java', 'points': 8}]
# print(dict_a)
# print(list(map(lambda x: x['name'], dict_a)))
# print(list(map(lambda x: x['points'] * 10, dict_a)))
# print(list(map(lambda x: x['name'] == 'python', dict_a))) # True, False

### filter - собирает и пропускает только то, что дает
# истину в его логическом выражении

# a = [3, 4, 5, 2, 7, 8]
# b = [7, 9, 2, 4, 5, 1]
# c = [5, 7, 3, 4, 5, 9]
#
# dict_a = [{'name': 'python', 'points':10}, {'name': 'java', 'points': 8}]
#
# print(list(filter(lambda x: x > 5, c))) # [7, 9]
# print(list(filter(lambda x: x['name'] == 'python', dict_a))) # [{'name': 'python', 'points':10}]

dict_b = [{"name": "John", "age": 12},
	      {"name": "Sonia", "age": 10},
	      {"name": "Steven", "age": 13},
	      {"name": "Natasha", "age": 9}]

names = ["Abram", "Arib", "Bob", "Shawn",
	         "Aria", "Cicilia", "John", "Reema",
	         "Alice", "Craig", "Aaron", "Simi"]
# print(sorted(names, key=lambda x: x.endswith('n')))
print(sorted(map(lambda x: x['age'], dict_b )))
print(sorted(dict_b, key=lambda x: x['age'], reverse=True))
print(*sorted(dict_b, key=lambda x: x['age'], reverse=True), sep='\n')
